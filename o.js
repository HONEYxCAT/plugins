(function () {
    ...
    function log(msg, data) {
        // Удобное отображение в консоли с префиксом VIP_DEBUG
        if (data) console.log('%c[VIP_DEBUG] ' + msg, 'color: #00ff00; background: #333; padding: 2px 5px;', data);
        else console.log('%c[VIP_DEBUG] ' + msg, 'color: #00ff00; background: #333; padding: 2px 5px;');
    }
    ...
    var MY_AUTH = {
        lampa_uid: 'jjhjf3yt',
        lampac_unic_id: 'guest',
        lampac_nws_id: '3jwwjn8mwyvtxmz3czoavx8mexim54a9'
    };

    log('Auth config loaded:', MY_AUTH);

    Lampa.Storage.set('lampac_unic_id', MY_AUTH.lampac_unic_id);
    Lampa.Storage.set('lampac_nws_id', MY_AUTH.lampac_nws_id);

    var balansers_with_search;
    
    // --- RCH (TUNNEL) ---
    var hostkey = 'lampa.vip';
    ...
    window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
        if (!window.rch_nws[hostkey].startTypeInvoke) {
            ...
            var check = function check(good) {
                ...
                log('RCH Check Result: ' + window.rch_nws[hostkey].type);
                call();
            };
    ...
    window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
        ...
        window.rch_nws[hostkey].typeInvoke('http://lampa.vip', function () {
            ...
            client.on('Connected', function (connectionId) {
                log('RCH Connected. ID: ' + connectionId);
    ...
    function account(url) {
        ...
        if (url.indexOf('nws_id=') == -1) {
            url = Lampa.Utils.addUrlComponent(url, 'nws_id=' + encodeURIComponent(MY_AUTH.lampac_nws_id));
        }
        if (url.indexOf('device_id=') == -1) {
            url = Lampa.Utils.addUrlComponent(url, 'device_id=' + encodeURIComponent(MY_AUTH.lampa_uid));
        }
        if (url.indexOf('token=') == -1) {
            // Исправление: не отправляем пустой токен, если его нет
            var token = ''; 
    ...
    function component(object) {
        ...
        // --- ИНИЦИАЛИЗАЦИЯ ШАБЛОНОВ ВНУТРИ КОМПОНЕНТА ---
        // Это решит проблему с ошибкой "Template not found"
        function initTemplates() {
            log('Initializing Templates...');
            if(window.lampavip_css_inited) return; // Чтобы не добавлять CSS много раз
            ...
            window.lampavip_css_inited = true;
        }
        ...
        this.initialize = function () {
            log('Initialize called', object);
            
            // --- ВАЖНО: Инициализируем шаблоны перед всем остальным ---
            initTemplates();
            // ---------------------------------------------------------
            ...
            if (object.balanser) {
                log('Initial balanser set: ' + object.balanser);
                ...
                log('Direct request for specific balanser:', reqUrl);

                return network["native"](reqUrl, function(response){
                    log('Direct response received');
                    this.parse(response);
        ...
        this.changeBalanser = function (balanser_name) {
            log('Changing balanser to: ' + balanser_name);
        ...
        this.requestParams = function (url) {
            ...
            var res = url + (url.indexOf('?') >= 0 ? '&' : '?') + query.join('&');
            return res;
        };
        ...
        this.startSource = function (json) {
            return new Promise(function (resolve, reject) {
                ...
                if (filter_sources.length) {
                    ...
                    log('Sources prepared. Active balanser:', balanser);
                    resolve(json);
                } else {
        ...
        this.request = function (url) {
            ...
            log('Requesting content URL:', finalUrl);
            
            if (number_of_requests < 10) {
                network["native"](finalUrl, function(response){
                    log('Response received. Length: ' + (response ? response.length : 0));
                    this.parse(response);
        ...
        this.parseJsonDate = function (str, name) {
            log('parseJsonDate called for selector: ' + name);
            var elems = [];
            
            if (typeof str !== 'string') {
                log('parseJsonDate error: input is not a string', typeof str);
                return [];
            }
            
            var html = $('<div>' + str + '</div>');
            var found = html.find(name);
            log('Found items via jQuery:', found.length);
            
            found.each(function (index) {
                var item = $(this);
                try {
                    var rawJson = item.attr('data-json');
                    if(!rawJson) {
                         log('Item #' + index + ' has no data-json attribute!');
                         return;
                    }
                    ...
                    log('Parse Error at item #' + index, e);
                }
            });
            
            log('Total valid elements extracted:', elems.length);
            return elems;
        };
        ...
        this.display = function (videos) {
            log('Display called. Videos count: ' + videos.length);
            var _this5 = this;
            this.draw(videos, {
        ...
        this.parse = function (str) {
            ...
            if (typeof str !== 'string') {
                log('Parse warning: input is not a string. Type: ' + typeof str);
            }
            ...
            try {
                ...
                log('Parsed Buttons count: ' + buttons.length);

                if (items.length == 1 && items[0].method == 'link' && !items[0].similar) {
                    log('Single link item detected, redirecting...');
                    ...
                    log('Filtered Videos: ' + videos.length);
                    log('Filtered Similar: ' + similar.length);
                    ...
                    if (videos.length) {
                        ...
                        log('No videos, no similar, no seasons found. Triggering doesNotAnswer.');
                        this.doesNotAnswer(json);
                    }
                }
            } catch (e) {
                log('Parse fatal error', e);
        ...
        this.draw = function (items) {
            ...
            // Защита от отсутствия шаблона, хотя initTemplates() должна решить это
            try {
                if (!object.balanser) scroll.append(Lampa.Template.get('lampac_prestige_watched', {}));
            } catch(e) {
                log('Template error in draw', e);
                // Попытка восстановить шаблоны и повторить
                initTemplates();
                if (!object.balanser) scroll.append(Lampa.Template.get('lampac_prestige_watched', {}));
            }
        ...
        this.empty = function () {
            log('Empty function called. Rendering empty state.');
        ...
        this.noConnectToServer = function (er) {
            log('noConnectToServer called', er);
            var html = Lampa.Template.get('lampac_does_not_answer', {});
            html.find('.online-empty__buttons').remove();
    ...
    function startPlugin() {
        ...
        var manifst = {
            ...
            version: '1.0.0',
            name: 'VIP Online',
            description: 'Просмотр онлайн через lampa.vip',
            component: 'lampavip',
        ...
        addSourceSearch('Онлайн VIP', 'spider');
